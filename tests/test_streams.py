...
async def test_stream_reader_no_resume_when_chunks_still_high(
    ...
    # resume_reading should NOT be called because chunk count is still
    # >= low_water_chunks
...
async def test_stream_reader_tiny_chunk_flood_is_throttled(
    protocol: mock.Mock,
) -> None:
    """A flood of tiny HTTP chunks must throttle the transport.

    Without chunk-count flow control an attacker can stream an unbounded
    number of tiny chunks through a default-limit reader: ``_size`` never
    reaches ``_high_water``, so reading is never paused and
    ``_http_chunk_splits`` grows without limit (CVE-2025-69229).
    """
    loop = asyncio.get_event_loop()
    ...
    # Emulate a transport that honours pause_reading() by stopping delivery.
    flood = 100000
    fed = 0
    ...
    # The flood was cut short by the chunk-count high water mark, and not by
    # the byte-based one (which is never reached with single-byte chunks).
    assert protocol._reading_paused is True
    assert fed < flood
